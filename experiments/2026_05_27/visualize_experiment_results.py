...
SUBEXPERIMENT_RE = re.compile(r"^(?P<config>sglang(?:-baseline)?)_gsm8k_c(?P<batch_size>\d+)_n(?P<num_samples>\d+)_fs(?P<few_shot>\d+)$")
...
class Subexperiment:
    ...
    aggregate_completion_throughput: float
    request_count: int
...
def load_request_metrics(request_path: Path) -> RequestMetrics:
    ...
    client_metrics = require_object(data, "client_metrics", request_path)
    ...
    e2e_seconds = require_number(
        client_metrics,
        f"client_metrics.{E2E_SECONDS_KEY}",
        E2E_SECONDS_KEY,
        request_path,
    )
...
def load_subexperiment(path: Path, batch_size: int) -> Subexperiment:
    ...
    try:
        ...
        raise ValueError(f"{summary_path}: invalid JSON: {exc}") from exc

    score_metrics = require_object(summary_data, "score_metrics", summary_path)
    gsm8k_accuracy = require_number(
        score_metrics,
    ...
    for request_path in request_paths:
        ...
        total_completion_tokens += request_metrics.completion_tokens
        wall_time_seconds = max(wall_time_seconds, request_metrics.e2e_seconds)
    ...
    return Subexperiment(
        ...
        aggregate_completion_throughput=total_completion_tokens / wall_time_seconds,
...
def plot_throughput_vs_tokens_per_forward(
    ...
    y_values = [
        subexperiment.aggregate_completion_throughput
        for subexperiment in experiment.subexperiments
    ]
    labels = [
    ...
    ax.set_ylabel("Completion Throughput (tokens/s)")
    ax.grid(alpha=0.3)
    ax.set_title(
...
def plot_sglang_threshold_global_avg_tokens_vs_gsm8k_acc(
    experiments: list[Experiment], output_dir: Path, dpi: int
) -> Path:
    sglang_experiments = sorted(
        (experiment for experiment in experiments if experiment.config == "sglang"),
        key=lambda experiment: float(experiment.threshold),
    )
    if not sglang_experiments:
        raise ValueError("No sglang experiments found for threshold scatter plot")

    x_values = [
        mean(mean(subexperiment.tokens_per_forward) for subexperiment in experiment.subexperiments)
        for experiment in sglang_experiments
    ]
    y_values = [
        mean(subexperiment.gsm8k_accuracy for subexperiment in experiment.subexperiments)
        for experiment in sglang_experiments
    ]
    labels = [experiment.threshold for experiment in sglang_experiments]
    ...
    ax.set_xlabel("global_average_tokens_per_forward")
    ax.set_ylabel("global_average_gsm8k_acc")
    ax.grid(alpha=0.3)
    ax.set_title("sglang thresholds: global avg tokens/forward vs global avg GSM8K acc")
    fig.tight_layout()

    output_path = output_dir / "sglang_threshold_global_avg_tokens_vs_gsm8k_acc.png"
    fig.savefig(output_path, dpi=dpi, bbox_inches="tight")
    plt.close(fig)
    print(f"Wrote {output_path} (sglang thresholds global avg tokens vs gsm8k acc)")
    return output_path
...
def main() -> None:
    ...
    written_paths.append(
        plot_sglang_threshold_global_avg_tokens_vs_gsm8k_acc(
